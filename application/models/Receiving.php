...
class Receiving extends CI_Model
{
	...
	function save ($cart)
	{
		...
		foreach($items as $line=>$item)
		{
			...
			// current quantity according to Item ID
			//*******************
			//
			$current_quantity_now=0;
			...
			if (!$cur_item_info->is_service)
			{
				...
				if ($item->variation_id)
				{
					...
					if ($inventory_to_add !=0)
					{
						...
						$current_quantity_now=($cur_item_variation_location_info->quantity ?  $cur_item_variation_location_info->quantity  : 0) + $inventory_to_add;
				...
				else
				{
					...
					if ($inventory_to_add !=0)
					{
						...
						$recv_remarks ='RECV '.$receiving_id;
						$current_quantity_now=$cur_item_location_info->quantity + $inventory_to_add;
						$inv_data = array
						(
							'trans_date'=>date('Y-m-d H:i:s'),
							'trans_items'=>$item->item_id,
			...
			{				
				...
				if ($item->variation_id)
				{
					...
					$current_quantity_now=$item_loc_var_to_save_qty;
				...
				else
				{
					...
					$current_quantity_now=$item_loc_qty_to_save;
					//Change values from $inv_data above and insert
					$inv_data['trans_inventory']=($item->quantity*($item->quantity_unit_quantity !== NULL ? $item->quantity_unit_quantity : 1)) * -1;
					$inv_data['location_id']=$location_id;
			...
			(
				...
				'current_quantity'=>$current_quantity_now,
		...
		foreach($this->db->get()->result_array() as $receiving_item_row)
		{
			...
			else
			{
				...
				if ($suspended == 0 && $receiving_item_row['transfer_to_location_id'])
				{
					...
					$this->Item_location->save_quantity($cur_item_location_transfer_info->quantity - $receiving_item_row['quantity_purchased'],$receiving_item_row['item_id'], $receiving_item_row['max_order_qty'], $receiving_item_row['transfer_to_location_id']);
