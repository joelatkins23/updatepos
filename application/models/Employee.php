	...
	*/
	...
	{	
		...
		$data=$this->db->query("SELECT *,${people}.person_id as pid 
						...
						WHERE ".$employees.".id<>1 and deleted =$deleted and $inactive and $location GROUP BY $people.person_id $order_by 
						LIMIT  ".$offset.",".$limit);		
		// end joel				
		return $data;
	}
	 ...
	 $this->db->select('employees_locations.location_id,employees.*,people.*,people.person_id as pid');
		...
		if ($search)
		{
			// update joel
				if ($search_field)
				{
					$this->db->where("$search_field LIKE '".$this->db->escape_like_str($search)."%' and deleted=$deleted and employees.id<>1");		
				}
				else
				{
					// update joel
					...
					full_name LIKE '".$this->db->escape_like_str($search)."%') and deleted=$deleted and employees.id<>1");		
