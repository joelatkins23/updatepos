	...
	*/
	function get_all($deleted=0,$limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		else
		{
			...
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->select($this->db->escape(lang('common_inv')).' as inventory, suppliers.company_name as supplier_company_name, location_items.location as location, items.*, COALESCE(phppos_items.main_image_id,phppos_item_images.image_id) as image_id, GROUP_CONCAT(DISTINCT '.$this->db->dbprefix('tags').'.name) as tags, categories.id as category_id,categories.name as category,
	...
	*/
	function get_item_search_suggestions_without_variations($search,$deleted = 0,$limit=25,$price_field = FALSE,$hide_inactive = false)
	{
		...
		else
		{
			...
			$this->db->select('items.*, COALESCE(phppos_items.main_image_id,phppos_item_images.image_id) as image_id, categories.name as category');
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
	...
	*/
	...
	function search($search, $deleted=0,$category_id = false, $limit=20,$offset=0,$column='name',$orderby='asc', $fields = 'all')
	{
		...
		else
		{
			...
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->select('"'.lang('common_inv').'" as inventory, suppliers.company_name as supplier_company_name,location_items.location as location, COALESCE(phppos_items.main_image_id,phppos_item_images.image_id) as image_id, GROUP_CONCAT(DISTINCT '.$this->db->dbprefix('tags').'.name) as tags,items.*,categories.id as category_id,categories.name as category,
				location_items.quantity as quantity,0 as variation_count, 0 as has_variations, 
				location_items.reorder_level as location_reorder_level,
				location_items.cost_price as location_cost_price,
				location_items.unit_price as location_unit_price,
