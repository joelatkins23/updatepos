...
class Item_location extends CI_Model
{
	...
	// update joel
	function save_quantity($quantity, $item_id, $max_order_qty, $location_id=false)
	{
		// echo $max_order_qty;
		if(!$location_id)
		{
		...
		if($max_order_qty){
			$sql1 = 'INSERT INTO '.$this->db->dbprefix('location_items'). ' (max_order_qty, item_id, location_id)'
					. ' VALUES (?, ?, ?)'
					. ' ON DUPLICATE KEY UPDATE max_order_qty = ?'; 
			$this->db->query($sql1, array($max_order_qty, $item_id, $location_id, $max_order_qty));	
		}
		return $this->db->query($sql, array($quantity, $item_id, $location_id, $quantity));	
	}
	function reset_expire_date($item_id){
		$this->db->where('item_id', $item_id);
		return $this->db->update('items', array('expire_days' => NULL,'last_modified' => date('Y-m-d H:i:s')));
