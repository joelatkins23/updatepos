...
{	
	function __construct()
	{
		...
		//Custom Coding Freelancer Starts
		 $this->load->model('InventorySummaryCustom_model'); //Load the Model here   
		//Custom Coding Freelancer Ends
...
if(isset($_GET['export_excel'])) {
    ...
    if( $this->Employee->has_module_action_permission('reports','view_inventory_at_all_locations',$this->Employee->get_logged_in_employee_info()->person_id) =="1" && $this->InventorySummaryCustom_model->checkIfPersonHasPermissionForInv() =="1" ) {
    ...
    $_GET['item_name']='';
     $_GET['supplier']=0;
		...
		if (!empty($get))		{ 
		  ...
		  $myDataOfLocations = $this->InventorySummaryCustom_model->myCustomLocationsData($get)  ;
		 
		  
		  if($get['export_excel']=="1") {
      ...
      header("Content-type: application/csv");
            ...
            header("Expires: 0");
              $handle = fopen('php://output', 'w');
                  fputcsv($handle, array("Daily Operational Report"));
                  $locationNamesArr=[];
                  $locationIdsArr=[];
     
             
                
                  $totalSalesMadeArr=["Total Sales Made"];
                  
 foreach($myDataOfLocations as $myDataOfLocation) {
    array_push($locationNamesArr,$myDataOfLocation->name);
       array_push($locationIdsArr,$myDataOfLocation->location_id);
 }
    $uniqueLocationNamesArr = array_unique($locationNamesArr);
     $uniqueLocationIdsArr   =        array_unique($locationIdsArr);
                array_unshift($uniqueLocationNamesArr,"","");
                array_push($uniqueLocationNamesArr,"Total");
              fputcsv($handle, $uniqueLocationNamesArr);   
              fputcsv($handle,[ "Revenue"]);
                  
              
              $totalSaleAmountsOfAllLocations=["","Total Sales Made"];
              $costToItemsSoldOfAllLocations=["","Cost To Items Sold"];
               $netRevenueOfAllLocations=["","Net Revenue"];
               
               $totalSaleAmountCombined=0;
               $totalCostAmountCombined=0;
               $totalNetRevenueCombined=0;
               
              foreach($uniqueLocationIdsArr as $uniqueLocationId) {
            
            $getTotalSaleAmountLocationFromModel=$this->InventorySummaryCustom_model->getTotalSaleForLocation($uniqueLocationId,$get['date']);
            
                array_push($totalSaleAmountsOfAllLocations, $getTotalSaleAmountLocationFromModel) ;
                
                
                $totalSaleAmountCombined=$totalSaleAmountCombined+$getTotalSaleAmountLocationFromModel;
                
                $getCostOfLocationFromModel=$this->InventorySummaryCustom_model->getCostToItemSoldForLocation($uniqueLocationId,$get['date']);
                
                
                    array_push($costToItemsSoldOfAllLocations, $getCostOfLocationFromModel) ;    
                    
                    
                    $totalCostAmountCombined=$totalCostAmountCombined+$getCostOfLocationFromModel;
                    
                    $getNetRevenueAmountFromModel=$this->InventorySummaryCustom_model->getNetRevenueForLocation($uniqueLocationId,$get['date']);
                    
                              array_push($netRevenueOfAllLocations, $getNetRevenueAmountFromModel) ;
                              
                              $totalNetRevenueCombined=$totalNetRevenueCombined+$getNetRevenueAmountFromModel;
              ...
              array_push($totalSaleAmountsOfAllLocations,$totalSaleAmountCombined);
                 array_push($costToItemsSoldOfAllLocations,$totalCostAmountCombined);
                    array_push($netRevenueOfAllLocations,$totalNetRevenueCombined);
                      fputcsv($handle, $totalSaleAmountsOfAllLocations);
                           fputcsv($handle, $costToItemsSoldOfAllLocations);
                  fputcsv($handle, $netRevenueOfAllLocations);
                   fputcsv($handle, ['']);
                  fputcsv($handle, ['Stock']);
                  
              
                  
                   $stockBalancesOfAllLocations=["","Stock Balances B/F (". date('Y-m-d', strtotime($get['date'] .' -1 day')).")"];
                   $stockBalancesCombined=0;
                   $newStockRecdOfAllLocations=["","New Stock Received"];
                $newStockRecdCombined=0;
                  $stockTransferOfAllLocations=["","Stock Transferred"];
                $stockTransferCombined=0;
                   $stockBalancesCDEachLocation=["","Stock Balances C/D"];
                   $stockBalancesCDCombined=0;
                            foreach($uniqueLocationIdsArr as $uniqueLocationId) {
            
            $getStockBalanceOfLocationFromModel=$this->InventorySummaryCustom_model->getStockBalanceForLocation($uniqueLocationId, date('Y-m-d', strtotime($get['date'] .' -1 day')));
       
                array_push($stockBalancesOfAllLocations, $getStockBalanceOfLocationFromModel) ;
                
                  $stockBalancesCombined=$stockBalancesCombined+$getStockBalanceOfLocationFromModel;
                  
                  
                  //Code for New stock Received
                  $getNewStockRecdOfLocationFromModel=abs($this->InventorySummaryCustom_model->getNewStockRecdOfLocation($uniqueLocationId,$get['date']));
                  
                     array_push($newStockRecdOfAllLocations, $getNewStockRecdOfLocationFromModel) ;
                  $newStockRecdCombined=$newStockRecdCombined+$getNewStockRecdOfLocationFromModel;
                  //Code for Stock Transfer
                       $getStockTransferOfLocationFromModel=$this->InventorySummaryCustom_model->getNewStockTransferOfLocation($uniqueLocationId,$get['date']);
                  
                     array_push($stockTransferOfAllLocations, $getStockTransferOfLocationFromModel) ;
                  $stockTransferCombined=$stockTransferCombined+$getStockTransferOfLocationFromModel;
                  
                  // C/D
                                  $getNewCostOfLocationFromModel=$this->InventorySummaryCustom_model->getCostToItemSoldForLocation($uniqueLocationId,$get['date']);
                  $setBalanceCDofLocation=$getStockBalanceOfLocationFromModel+$getNewStockRecdOfLocationFromModel-$getStockTransferOfLocationFromModel-$getNewCostOfLocationFromModel;
                  array_push($stockBalancesCDEachLocation,$setBalanceCDofLocation);
                  $stockBalancesCDCombined=$stockBalancesCDCombined+$setBalanceCDofLocation;
                  
                
                            }
                  
                      array_push($stockBalancesOfAllLocations,$stockBalancesCombined);
                   array_push($newStockRecdOfAllLocations,$newStockRecdCombined);
                     array_push($stockTransferOfAllLocations,$stockTransferCombined);
                     fputcsv($handle, $stockBalancesOfAllLocations);
                     
                        fputcsv($handle, $newStockRecdOfAllLocations);
                        fputcsv($handle, $stockTransferOfAllLocations);
                     
                     
                     
                     
                     
                     $totalStockSoldByEachLocation=$costToItemsSoldOfAllLocations;
                     $totalStockSoldByEachLocation[1]="Stock Sold";
                        fputcsv($handle, $totalStockSoldByEachLocation);
                       
                        array_push($stockBalancesCDEachLocation,$stockBalancesCDCombined);
                        
                        fputcsv($handle, $stockBalancesCDEachLocation);
...
//Custom Coding Freelancer ends
	...
	if( $this->Employee->has_module_action_permission('reports','view_inventory_at_all_locations',$this->Employee->get_logged_in_employee_info()->person_id) =="1" && $this->InventorySummaryCustom_model->checkIfPersonHasPermissionForInv() =="1" ) {
		//custom code
 unset($data['input_data']['input_params'][0]);
  unset($data['input_data']['input_params'][1]);
    unset($data['input_data']['input_params'][2]);
        unset($data['input_data']['input_params'][3]);
            unset($data['input_data']['input_params'][4]);
        unset($data['input_data']['input_params'][6]);
 
      $data['input_data']['input_params'][1]=array('view'=>'date');
      	
          unset($data['input_data']['input_params'][8]);
      
         
        
                      sort($data['input_data']['input_params']);
    $data['input_data']['input_params'][3] =array("view"=>"submit") ;
	}
	else {
	        echo "Not Authorised";
	    exit;
...
if( $this->Employee->has_module_action_permission('reports','view_inventory_at_all_locations',$this->Employee->get_logged_in_employee_info()->person_id) =="1" && $this->InventorySummaryCustom_model->checkIfPersonHasPermissionForInv() =="1" ) {
      	    	  ...
      	    	  $myCustomDataOfLocations = $this->InventorySummaryCustom_model->myCustomLocationsData($get)  ;
	    		     $customLocationIdsArr=[];
	    		                    
 foreach($myCustomDataOfLocations as $myCustomDataOfLocation) {
 //   array_push($customLocationIdsArr,$myCustomDataOfLocation->name);
       array_push($customLocationIdsArr,$myCustomDataOfLocation->location_id);
 }
	    	     $customUniqueLocationIdsArr   =        array_unique($customLocationIdsArr);
	    	     
	    	     $dash_combined_total_sales_made=0;
	    	     $dash_combined_cost_to_items_sold=0;
	    	     $dash_combined_net_revenue=0;
	    	     $dash_combined_stock_balances_BF=0;
	    	     $dash_combined_new_stock_recd=0;
	    	     $dash_combined_stock_transfer=0;
	    	     $dash_combined_stock_sold=0;
	    	     $dash_combined_stock_balances_CD=0;
	    	     foreach($customUniqueLocationIdsArr as $customUniqueLocationId) {
	    	      //   echo $customUniqueLocationId.date('Y-m-d', strtotime($get['date'] .' -1 day'));
	    	      //   echo "<br>";
	    	         $dash_combined_total_sales_made=$dash_combined_total_sales_made+$this->InventorySummaryCustom_model->getTotalSaleForLocation($customUniqueLocationId,$get['date']);
	    	         
	    	         $dash_combined_cost_to_items_sold=$dash_combined_cost_to_items_sold+$this->InventorySummaryCustom_model->getCostToItemSoldForLocation($customUniqueLocationId,$get['date']);
	    	         
	    	         $dash_combined_net_revenue=$dash_combined_net_revenue+$this->InventorySummaryCustom_model->getNetRevenueForLocation($customUniqueLocationId,$get['date']);
	    	         
	    	         $dash_combined_stock_balances_BF=$dash_combined_stock_balances_BF+$this->InventorySummaryCustom_model
	    	         ->getStockBalanceForLocation($customUniqueLocationId,date('Y-m-d', strtotime($get['date'] .' -1 day')));
	    	 ;
	    	         $dash_combined_new_stock_recd=$dash_combined_new_stock_recd+abs($this->InventorySummaryCustom_model->getNewStockRecdOfLocation($customUniqueLocationId,$get['date']));
	    	         
	    	         $dash_combined_stock_transfer=$dash_combined_stock_transfer+$this->InventorySummaryCustom_model->getNewStockTransferOfLocation($customUniqueLocationId,$get['date']);
      	            ...
      	            $dash_combined_stock_sold=+$dash_combined_stock_sold+ $dash_combined_cost_to_items_sold;
	    	         	    	     
	    	         $dash_combined_stock_balances_CD=+$dash_combined_stock_balances_CD+$dash_combined_stock_balances_BF+$dash_combined_new_stock_recd-$dash_combined_stock_transfer-$dash_combined_stock_sold ;
	    	         
      	    
      	    
      	    
 unset($data['output_data']);
$data['output_data']= ['view'=>'inventory_summary_custom_layout'];

 

$data['output_data']['details_data']=  [
    "dash_combined_total_sales_made"=>$dash_combined_total_sales_made,
    "dash_combined_cost_to_items_sold"=>$dash_combined_cost_to_items_sold,
    "dash_combined_net_revenue" => $dash_combined_net_revenue, "dash_combined_stock_balances_BF" => $dash_combined_stock_balances_BF, "dash_combined_new_stock_recd"=>$dash_combined_new_stock_recd , "dash_combined_stock_transfer"=>$dash_combined_stock_transfer, "dash_combined_stock_sold"=>$dash_combined_stock_sold, "dash_combined_stock_balances_CD"=>$dash_combined_stock_balances_CD ];
}
else {
        echo "Not Authorised";
    exit;
