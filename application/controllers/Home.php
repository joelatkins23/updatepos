...
class Home extends Secure_area 
{
	...
	{		
		...
		// $this->db->select('locations.name as location_name, items.name, receivings_items.current_quantity as quantity_expiring, items.size,receivings_items.expire_date, categories.id as category_id,categories.name as category, company_name, item_number, product_id, 
		...
		$sql="SELECT ww.* FROM (SELECT MAX(receiving_id)receiving_id FROM phppos_receivings_items GROUP BY item_id ORDER BY receiving_id DESC) w 
		JOIN (SELECT phppos_items.item_id, phppos_receivings_items.receiving_id, phppos_locations.name AS location_name, phppos_items.name, phppos_receivings_items.current_quantity AS quantity_expiring, phppos_items.size,phppos_receivings_items.expire_date, phppos_categories.id AS category_id,phppos_categories.name AS category, company_name, item_number, product_id, phppos_receivings_items.item_unit_price AS cost_price, IFNULL(phppos_location_items.unit_price, phppos_items.unit_price) AS unit_price, IFNULL(phppos_location_items.reorder_level, phppos_items.reorder_level) AS reorder_level,  phppos_items.description, FALSE 
		FROM phppos_items 
		JOIN phppos_receivings_items ON phppos_receivings_items.item_id = phppos_items.item_id 
		JOIN phppos_receivings ON phppos_receivings_items.receiving_id = phppos_receivings.receiving_id 
		...
		JOIN phppos_locations ON phppos_locations.location_id = phppos_receivings.location_id 
		LEFT JOIN phppos_location_items ON phppos_location_items.item_id = phppos_items.item_id AND phppos_location_items.location_id = '$current_location_id'
		WHERE phppos_items.deleted=0 
		AND phppos_items.system_item=0 
		AND phppos_receivings.location_id = '$current_location_id'
		AND phppos_receivings_items.expire_date >='$start_date' 
		AND phppos_receivings_items.expire_date <='$end_date' 
		ORDER BY phppos_receivings_items.receiving_id DESC) ww ON w.receiving_id=ww.receiving_id ORDER BY receiving_id DESC";

		$expire_result = $this->db->query($sql)->result_array();
