		...
		<div class="col-md-12">
			<div class="panel panel-piluku">
		      ...
		      <h3 class="panel-title"><i class="ion-android-clipboard"></i> <?php echo lang("common_inventory"); ?> <small>(<?php echo lang('common_fields_required_message'); ?>)</small></h3>
				...
				<!-- create Jole -->
				<?php
					if( $item_info->reorder_level || $item_info->item_id){
						$aaa=to_quantity($item_info->reorder_level, FALSE);
					}else{
						if($this->config->item('reorder_level_select')=='fixed_value'){
							$aaa=$this->config->item('default_reorder_level_when_creating_items');
						}else{
							$aaa=$this->config->item('default_percentage_when_creating_items');
						}
					}
				?>
				<!-- End Jole -->	
					...
					<!-- create Joel-->
					<div class="form-group">
						<?php echo form_label(lang('max_order_quantity').':', null,array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label')); ?>
						<div class="col-sm-9 col-md-9 col-lg-10 form-text">
							<?php echo form_input(array(
								'name'=>'max_order_qty',
								'id'=>'max_order_qty',
								'placeholder' => to_quantity(null),
								'data-original' => (to_quantity($item_location_info->max_order_qty)!=0) ? to_quantity($item_location_info->max_order_qty) : $this->config->item('default_price_when_creating_items'),
								'value' => (to_quantity($item_location_info->max_order_qty)!=0) ? to_quantity($item_location_info->max_order_qty) : $this->config->item('default_price_when_creating_items'),
								'class'=>'form-control'
								)
							);?>
						</div>
					...
					<table class="table table-striped table-hover custom-table">
						<thead>
							<tr>
								...
								<!-- <th><?php echo lang("max_order_quantity"); ?></th> -->
						...
						<tbody>
							<?php foreach($inventory_data as $row) { ?>
								<tr>
									...
									<!-- <td><?php echo to_quantity($row['trans_max_order_qty']);?></td> -->
									<?php
...
{	
	...
	$('#item_form').validate({
		...
		messages: 
		{
			...
			// create Jole
			max_order_qty: {
				customRule: jQuery.validator.format("Current quantity must not be greater than the maximum order quantity.")
			}
			// end Jole
		}
	});
...
$.validator.addMethod("customRule",function(value,element){	
   if(parseInt($('#max_order_qty').val()) >= parseInt($('#current_qty').val())){
		return true;
	}else{
		return false;
	}
},"Current quantity must not be greater than the maximum order quantity.");
// end Jole
