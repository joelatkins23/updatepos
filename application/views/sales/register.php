									...
									<dl class="register-item-extra-details dl-horizontal">
									  ...
									  <dd  class="">
										...
										<?php if (isset($item->item_id) && $item->item_id) 
											$expire_date='';
											{ 											
												$item_expire_date_data = $this->Sale->item_expire_date_data($item->item_id);
												$expire_date_val=$this->Sale->get_item_expire_date($item->item_id);
												foreach($item_expire_date_data as $arr){
													$expire_date.= date_format(date_create($arr['expire_date']),'d-m-Y').', ';

													$dif=(strtotime($arr['expire_date'])-strtotime(date('Y-m-d')))/(3600*24);

													if($dif>0 && $dif < $expire_date_val[0]['expire_days']){
														$warning1= H($item->name).' <br> '.date_format(date_create($arr['expire_date']),'d-m-Y');
														echo '<script type="text/JavaScript">  
																show_feedback("warning", "'.$warning1.'", '.json_encode(lang('common_warning')).'); 
															</script>'; 
													} else if ($dif <= 0) {
															$error1=H($item->name).' <br> '.date_format(date_create($arr['expire_date']),'d-m-Y');
															echo '<script type="text/JavaScript">  
																show_feedback("error", "'.$error1.'", '.json_encode(lang('common_error')).'); 
																</script>'; 
...
<?php
if (isset($number_of_points_to_use) && $number_of_points_to_use > 0 && $this->config->item('prompt_to_use_points') && $number_of_points_to_use >= $this->config->item('minimum_points_to_redeem'))
{
?>
<script>
bootbox.confirm({
